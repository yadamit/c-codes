...
for i in freq_total:
    if i in freq_rcvd:
        prob_rcving[i]=(freq_rcvd[i]+1.0)/(rcvd_words)*1000
    else:
        prob_rcving[i]=(1.0)/(rcvd_words)*1000
    if i in freq_not_rcvd:
        prob_not_rcving[i]=(freq_not_rcvd[i]+1.0)/(not_rcvd_words)*1000
    else:
        prob_not_rcving[i]=(1.0)/(not_rcvd_words)*1000

prob_rcv=(got_piza*1.0)/(got_piza+not_get_piza)
prob_not_rcv=(not_get_piza*1.0)/(got_piza+not_get_piza)
#=============================================



def predict(request):
# take a input as text and output if he will recieve a pizza or not
...
with open('prediction.csv','wb') as fo:
    ...
    #please manually add the header in output file
    w=csv.writer(fo)
